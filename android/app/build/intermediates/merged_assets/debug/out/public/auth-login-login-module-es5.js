(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["auth-login-login-module"], {
  ...
  function srcAppAuthLoginLoginPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    let LoginPage = class LoginPage {
      ...
      onSubmit() {
        if (this.loginForm.valid) {
          this.auth.loginStrapiUser(this.loginForm).subscribe(response => {
            console.log(response);

            if (response.jwt) {
              this.storage.set('token', response.jwt).then(() => this.storage.set('user', response.user)).then(() => this.router.navigateByUrl('calculator-slides')), error => console.log(error);
            }
          });
        } else {
          console.log('Error');
    ...
    }), tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"]("design:paramtypes", [_angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormBuilder"], src_app_services_auth_service__WEBPACK_IMPORTED_MODULE_4__["AuthService"], _angular_router__WEBPACK_IMPORTED_MODULE_3__["Router"], _ionic_storage__WEBPACK_IMPORTED_MODULE_5__["Storage"]])], LoginPage);
    /***/
  }
}]);
